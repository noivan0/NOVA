...
Commands:
  ...
  nova init                    Copy example harnesses + nova.yaml into cwd
...
def main() -> None:
    ...
    init_p = sub.add_parser(
        "init",
        help="Copy example harnesses + nova.yaml into the current directory "
             "(no git clone needed after `pip install`)",
    )
    init_p.add_argument(
        "--harnesses", nargs="*", default=None,
        help="Which example harnesses to copy (default: research summarizer data-pipeline)",
    )
    init_p.add_argument(
        "--all", action="store_true",
        help="Copy every bundled example harness instead of the curated starter set",
    )
    init_p.add_argument(
        "--force", action="store_true",
        help="Overwrite files that already exist in the current directory",
    )
...
_INIT_DEFAULT_HARNESSES = ["research", "summarizer", "data-pipeline"]

_INIT_NOVA_YAML = """\
# nova.yaml — NOVA configuration file
# Generated by `nova init`. All values can be overridden with NOVA_* env vars.

workspace: ./workspace          # Where harness run artifacts are stored
harnesses_dir: ./harnesses      # Where harness directories live

llm:
  provider: echo                 # echo | openai | anthropic | ollama | custom
  model: gpt-4o
  api_key: ""                    # Or set NOVA_LLM_API_KEY env var
  max_tokens: 4096
  temperature: 0.7
  timeout: 120
...
def _cmd_init(args) -> None:
    """Copy bundled example harnesses + a starter nova.yaml into cwd.

    P0 install fix: after a fresh `pip install nova-orchestrator` there is no
    ./harnesses directory and no nova.yaml in the user's project — `nova init`
    materializes both from the package data shipped inside the wheel, so a
    harness can be run immediately with zero network access / git clone.
    """
    import shutil

    from nova.core.harness import _packaged_harnesses_dir

    packaged = _packaged_harnesses_dir()
    if packaged is None or not packaged.exists():
        print("Error: could not locate bundled example harnesses inside the "
              "installed nova package. Is nova installed correctly?")
        sys.exit(1)

    available = sorted(
        p.name for p in packaged.iterdir() if p.is_dir() and (p / "harness.yaml").exists()
    )

    if getattr(args, "all", False):
        wanted = available
    elif args.harnesses:
        # Security: `name` ends up in both a source (`packaged / name`) and a
        # destination (`dest_root / name`) path that is later `shutil.rmtree()`'d
        # when overwriting with --force. Without validation, `--harnesses ..`
        # makes `dest_root / ".."` resolve to the CWD itself (or an ancestor of
        # it) and `--force` would then recursively delete it. Only accept exact
        # names from the enumerated whitelist of bundled harnesses — no path
        # separators, no `.`/`..`, no absolute paths can ever reach the
        # filesystem ops below.
        wanted = []
        rejected = []
        for h in args.harnesses:
            if h in available:
                ...
                rejected.append(h)
        if rejected:
            print(f"Ignoring unknown/unsafe harness name(s): {', '.join(rejected)} "
                  f"(must be one of: {', '.join(available)})")
    else:
        wanted = [h for h in _INIT_DEFAULT_HARNESSES if h in available]

    dest_root = Path.cwd() / "harnesses"
    ...
    for name in wanted:
        src = packaged / name
        dst = dest_root / name
        # Defense in depth: even though `name` is whitelist-validated above,
        # confirm the resolved destination is actually a direct child of
        # dest_root before any deletion/copy so a future whitelist bug can't
        # turn into a path-traversal file-system write.
        try:
    ...
    print(f"Bundled harnesses available: {', '.join(available)}")
    if copied:
        print(f"Copied to ./harnesses/: {', '.join(copied)}")
    if skipped:
        print(f"Skipped (already exists, use --force to overwrite): {', '.join(skipped)}")
    if missing:
        print(f"Warning — not found in package: {', '.join(missing)}")

    nova_yaml = Path.cwd() / "nova.yaml"
    if nova_yaml.exists() and not getattr(args, "force", False):
        print(f"Skipped nova.yaml (already exists, use --force to overwrite)")
    else:
        nova_yaml.write_text(_INIT_NOVA_YAML)
        print(f"Wrote {nova_yaml} (llm.provider=echo — works with zero API keys)")

    if copied:
        demo = copied[0]
        print()
        print("Try it now (no API key required):")
        print(f'  nova list')
        print(f'  nova run {demo} --context topic="test" --dry-run')
        print(f'  nova run {demo} --context topic="test"   # uses the echo provider')
